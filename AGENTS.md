...
This is the **base layer** other addons build on. The main consumer is `AmyJeanes/Doors` (TARDIS-style doors): it builds exterior↔interior teleporting on `linked_portal_door` and uses `wp.TransformPortal*` for view/velocity transforms. Doors' CI combines both repos into one Workshop upload, so paths here can collide with Doors at upload time (consumer wins) — hence no `addon.json`. `.github/workflows/ci.yml` runs `glua_check` on push/PR to `main`.

**Publishing a `main` change to the Workshop is automatic.** This repo has no Workshop upload of its own — the beta addon publishes from Doors' `main` CI, which bundles the world-portals `main` it checks out at that time. A green push to world-portals `main` fires the `trigger-doors` job in `ci.yml`, which dispatches Doors' beta publish (`workflow_dispatch` on Doors `ci.yml`) via the `TOKEN` PAT — skipped when a Doors `main` build has already run since this commit, so a coordinated Doors push isn't double-published.
...
### Contraption teleport (rigid groups) — `linked_portal_door/init.lua` + `sv_collision.lua`

A welded/roped contraption crosses as **one rigid body**, not prop-by-prop: `ENT:Touch` gathers the crossing prop's constraint network with `wp.GatherRigidGroup` and applies the same portal transform to every member **in one tick**. VPhysics solves constraints once per tick *after* all Lua, so moving every member together means the solver never sees a half-crossed contraption to snap — the old per-prop teleport is exactly what tore contraptions apart (issue #14). The transform is a rigid isometry, so relative poses (hence welds) survive by construction.

- **`wp.GatherRigidGroup(seed, portal)`** (in `sv_collision.lua`) walks **rigid edges only** — it skips `NoCollide` (binds no relative motion, and our own pass-through no-collides would otherwise bridge the mount into the set). It reads `constraint.GetTable` (`con.Entity[i].World`), since `constraint.GetAllConstrainedEntities` silently drops the world. Returns nil to **veto** the whole move when the group reaches the world (can't move the map) or map machinery (anything failing `wp.IsPhysicalMover`, e.g. a shadow-driven `func_door` — moving it would derail it; leaving it would re-create the snap), the portal's own mount (`wp.RidesPortal`), or a member `wp-shouldtp` rejects (atomic — deny one, deny all). A constrained **player** neither joins nor vetoes — the walk stops there, so a player-roped prop keeps its per-body crossing (players teleport only via the predicted path).
- **Trigger:** the same `worldportals_teleport_fraction` cvar as a lone prop, generalised to the whole contraption (member OBB corners projected onto the portal normal give the combined depth + centre; mass-weighted momentum gates direction). A lone prop (`#group <= 1`) keeps the exact single-body path unchanged.
- **Re-fire guard:** a group teleport stamps every member `wpGroupTpAt`/`wpGroupTpExit`; for ~0.2s a member can't re-cross the portal it just emerged from, killing the bounce a same-facing pair flings back into its exit. It's keyed to that **exit** (not time-global), so a vertical loop — which re-enters the *entry* portal — keeps firing rather than being throttled or falling straight through the non-solid opening. A per-tick memo (`wpGroupTick`/`wpGroupPortal`) evaluates the group once per tick per portal (Touch fires per member). All group-only — lone props are never stamped, so the `cl_renderfollow` rapid loop is untouched.

**Accepted / don't re-attempt:** at the default 0.9 a long contraption pokes further out the entry before the group jumps (lower the cvar to jump earlier) — inherent to the fraction, not a bug. The `eligible()` no-collide *pollution* (once one member arms the mount, the shell's constraint network reaches the whole welded group, so `eligible` refuses to re-arm the rest) is **theoretical**: the realistic mounted case transits fine — a welded contraption passes a real TARDIS doorway rather than pressing the solid shell — so no speculative "arm every member" code was added; revisit only if a big contraption is actually seen jamming on a shell. A post-teleport speed drop is ordinary physics drag over the longer flight, not the velocity transform (which preserves magnitude).
...
- **Records** (`wp.ghosts[ent]`) carry a 0.1s anti-flicker grace, are re-pointed to the new pair the instant the entity teleports (the `wp-teleport` hook updates `rec.portal/exit` before render, killing a one-frame flicker), and tear down on expiry/`EntityRemoved` (restoring the saved `RenderOverride`). The entry-half override draws through the chained `savedRenderOverride` if there is one, else the entity's own `ENT:Draw`, else `DrawModel` — a `RenderOverride` stands in for `ENT:Draw`, so a scripted entity would otherwise lose its whole draw chain and render as a bare model for as long as it straddles. `wp.IsGhosting(ent)` reports whether a record exists - a consumer that also drives the entity's `RenderOverride` (Doors' cordon) yields the slot while it's true.
...
**Collision frame (`linked_portal_frame`).** Invisible server-built perimeter hull (`FrameSlabs` → 4 box slabs via `PhysicsInitMultiConvex`) funnelling a transiting prop through the opening while the parent is "removed" for it. `COLLISION_GROUP_INTERACTIVE_DEBRIS`: hits props (funnelling them through the opening), blocks bullet/use traces, and passes both players and `COLLISION_GROUP_PLAYER_MOVEMENT` — the group NPC pathfinding traces present as — so NPC nav routes straight through the doorway. No `EnableCustomCollisions` (physics-vs-physics suffices; ECC would block bullet/use traces). Two near-misses to avoid: `COLLISION_GROUP_WEAPON` has the same prop/trace behaviour but its pass-list misses `PLAYER_MOVEMENT`, so it walls NPC nav out; `COLLISION_GROUP_DEBRIS` passes the NPC body too but is transparent to bullet/use traces. Created/resized by `RebuildCollisionFrame`/`BuildFrame`. Two verified choices:
