...
- **Comments: concise, why-not-what.** A couple of lines; reserve length for genuinely non-obvious rationale. Don't restate the code. Keep the *why* inline and self-contained — don't point a reader at an external doc (`memory/`, this file). The hard-won why is worth keeping; the wall of prose around it isn't. For a dash in a comment use a single spaced hyphen; never a double `--` (reads as a second comment marker) or an em-dash.
