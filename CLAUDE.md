...
8. On first-time-predicted client only — **never on the server** — call `ply:SetEyeAngles(clampedAng)`. `ply:SetEyeAngles` writes the player's persistent `m_angEyeAngles` field. Locally on the predicting client it's what actually rotates the camera (`cmd:SetViewAngles` alone only mutates the input cmd struct, leaving the camera on its last-input value — directional portals appeared to no-op without this). Calling it during **client resim** would clobber any mouse delta accumulated since (camera "snaps back" mid-teleport whenever you try to look around). Calling it on the **server** writes the networked eye-angle field, which the snapshot system pushes back to the owning client when the server catches up — same snap-back symptom, delayed by ~RTT. The server doesn't need to set it explicitly: subsequent cmds from the client carry the post-teleport angle (client predicted `clampedAng`, so next-frame mouse samples are relative to it), and the server reads those via `cmd:GetViewAngles`. For the one tick of the teleport cmd, `mv:SetAngles` gives gamemovement the right W/A/S/D direction; server-side `GetEyeAngles` is briefly stale but isn't read by anything in this path.
